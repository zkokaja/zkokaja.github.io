...
Carroll. The Fox and the Hedgehog are looking at an image of the galaxies found
by the [Hubble Telescope](https://www.spacetelescope.org/images/heic0406a/).
...
_Hedgehog_: "It is very useful and appears in many fields, including signal
processing and image processing (compression, segmentation). Now, for our
picture, we can think of each bright pixel as a pebble, and we want to find the
means of as many as we can."
...
initially. We have roughly 15,000 data points to begin with, so maybe we should
choose a smaller k."
...
algorithm like EM we could represent clusters with as a Gaussian, so we would
have a estimate variance as well. Maybe for next time, but for now I think you
have a good idea of k-means, Fox!"
...
I wanted to share some of the code I used for the post. This is a non-vectorized
implementation because it's easier to understand as an introduction. Here is the
assignment step:
